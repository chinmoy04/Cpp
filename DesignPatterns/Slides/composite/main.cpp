...
int open_audio_file(const char *szfilename, FILE **fp)
{
	if (szfilename == NULL)
	{
...
int read_audio_file_header(FILE *fp, uint8_t *header_data, uint8_t *audio_data)
{
	size_t read_sz = fread(header_data, sizeof(uint8_t), HEADER_LEN, fp);
...
int main(int argc, char* argv[])
{
	...
	if (accept_file_name(szfilename))
		return -1;

	if (open_audio_file(szfilename, &fp))
		return -1;

	if (read_audio_file_header(fp, header_data, audio_data))
		return -1;

	if (validate_pcm_wav_file(header_data))
		return -1;
